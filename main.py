...
def main():
    ...
    mc = MonteCarlo(S=S, K=K, T=T, r=r, sigma=sigma, n_simulations=100_000)

    print("\n MONTE CARLO vs BLACK-SCHOLES")
    print(f" Simulations : {mc.n_simulations:,}")
    print(f" Call  BS    : {mc.call_price():.4f}")
    print(f" Call  MC    : {mc.call_price_mc():.4f}")
    print(f" Put   BS    : {mc.put_price():.4f}")
    print(f" Put   MC    : {mc.put_price_mc():.4f}")
